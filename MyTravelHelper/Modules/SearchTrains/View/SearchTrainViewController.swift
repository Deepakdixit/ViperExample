...
class SearchTrainViewController: UIViewController {
    ...
    private func showAlert() {
        let alert = UIAlertController(title: "Error", message: "You need to provide source and destination.", preferredStyle: UIAlertController.Style.alert)
...
extension SearchTrainViewController:UITextFieldDelegate {
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        if let inputedText = textField.text {
            ...
            dropDown.dataSource = stationsList.filter { $0.stationDesc.lowercased().contains(desiredSearchText) }.map { $0.stationDesc }
            dropDown.show()
